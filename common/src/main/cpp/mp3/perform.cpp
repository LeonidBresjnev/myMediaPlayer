...
#include <math.h>
/* 138 */
     static mp3_sample mk_sample (double x)
{
  if (fabs (x) <= ((double) ((unsigned) 1 << OUTPUT_EXPONENT))       /* 136 */
    ...
    if (sizeof (mp3_sample) == sizeof (int))
      ...
      constexpr int d = (sizeof (int) - sizeof (mp3_sample)) * 8;
...
void windowing (const double *v, mp3_sample * x)
{ {                                                                  /* 412 */
    x[2 * 0] =
...
void dct32 (const double *y, double *v)
{
  {
...
void dct18 (const double *I, double *O) {
